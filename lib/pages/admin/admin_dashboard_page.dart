  ...
  Future<void> _handleStatusChange(VerificationRequest req, String newStatus, {String? reason}) async {
    ...
    if (newStatus == 'Approved') {
      if (req.type == 'Owner' && req.username != null) {
        ...
        // Avoid duplicate venue additions if already exists
        final bool exists = GlobalVenueData.venues.any((v) => v['name'] == newVenue['name']);
        if (!exists) {
          await GlobalVenueData.addVenue(newVenue);
        }
        ...
        if (req.username != null) {
          GlobalNotificationData.addNotification(AppNotification(
            ...
            message: 'Venue "${req.venueName}" Anda telah disetujui oleh Admin dan sekarang sudah aktif!',
        ...
        GlobalNotificationData.addNotification(AppNotification(
          ...
          message: 'Venue "${req.venueName}" sekarang sudah tersedia untuk dibooking di RENSIUS!',
          timestamp: DateTime.now(),
          icon: Icons.stadium,
          color: Colors.blue,
        ));
      ...
      if (req.type == 'Owner' && req.username != null) {
        ...
        if (req.username != null) {
          GlobalNotificationData.addNotification(AppNotification(
            ...
            username: req.username!,
            title: 'Venue Ditolak',
            message: 'Venue "${req.venueName}" Anda ditolak oleh Admin. Alasan: ${reason ?? "-"}',
            timestamp: DateTime.now(),
            icon: Icons.cancel,
            color: Colors.red,
          ));
